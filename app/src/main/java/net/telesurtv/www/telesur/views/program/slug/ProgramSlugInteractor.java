 ...
 */
public class ProgramSlugInteractor {


    private TelesurApiService telesurApiService;

    public ProgramSlugInteractor(TelesurApiService telesurApiService) {
        this.telesurApiService = telesurApiService;
    }

    public void getSlugFromServer(TelesurProgramSlugCallback telesurProgramSlugCallback) {

        telesurApiService.getProgramList("100", "nombre", "slug")
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(Programs -> { telesurProgramSlugCallback.onLoaderNews(getProgramItems(Programs));},
                        throwable -> {try{
                            telesurProgramSlugCallback.onFaliedToGetData(throwable.getMessage());}catch (Exception e){e.printStackTrace();}});
    }
